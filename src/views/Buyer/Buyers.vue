<template>
  <v-app id="lateral">
    ...
    <v-toolbar flat class="white">
      ...
      <h1 v-if="!searchFlag" class="display-1 justify-center primary--text font-weight-bold" >Buyers</h1>
      <v-spacer></v-spacer>
      <v-btn icon>
        <v-icon color="md-deep-orange-500" @click="searchFunc()">{{ searchFlag ? 'close' : 'search'}}</v-icon>
      </v-btn>
      <v-tabs slot="extension" v-model="tabs" color="" grow class="mt-2 primary--text">
          ...
          <v-tab-item key="two" value="two" >
            ...
            <v-flex xs10 centered mt-5 pt-4>
              <v-card flat>
                ...
                <v-card-text class="darken2 grey--text font-weight-bold">
                  <small>Create groups above to allow targeting of buyers.</small>
                </v-card-text>
              </v-card>
            </v-flex>
            <v-card min-height="100px" flat>
