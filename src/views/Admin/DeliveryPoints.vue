            ...
            <v-card-text class="pa-0 delivery-list-section">
              <div>
                <v-list two-line class="v-menu-trans-list-npd">
                  <template v-for="(item, index) in deliveryPoints">
                    <v-list-tile @click="editDelivery(item)" :key="`${index}-listdelivery`">
                      ...
                      <v-list-tile-action
                        @click.stop="togglePublicVisibility(item)"
                        class="compact-action"
                      >
                        <v-icon
                          :color="item.publicVisibility == 'primary' ? 'amber' : ''"
                        >{{ item.publicVisibility == 'primary' ? 'label_important' : 'location_on' }}</v-icon>
                      </v-list-tile-action>
    ...
    <v-dialog v-model="deliveryDialog" scrollable persistent max-width="480px">
      <v-card class="create-dialog-card">
        <v-card-title>
          <v-layout>
            <v-flex row xs6>
              <span class="white--text" v-if="isEditformMod">Edit Delivery Point</span>
              <span class="white--text" v-else>New Delivery Point</span>
            </v-flex>
...
export default {
  ...
  data() {
    return {
      ...
      activeMenu: "all",
      menuItems: [
        { title: "All Points ", isActive: true, slug: "all" },
        { title: "Primary Points", isActive: false, slug: "primary" },
  ...
  methods: {
    ...mapActions("deliveryPoint", {
      ...
      getPublicDeliveryPoints: "getPublicDeliveryPoints",
      addDeliveryPoint: "addDeliveryPoint",
      updateDeliveryPoint: "updateDeliveryPoint",
      publishDeliveryPoint: "publishDeliveryPoint",
      unpublishDeliveryPoint: "unpublishDeliveryPoint",
      deleteDeliveryPoint: "deleteDeliveryPoint",
      togglePublicVisibilityOfDeliveryPoint: "togglePublicVisibilityOfDeliveryPoint"
    }),
    deleteSelectedItems() {
      this.deleteLoading = true;
      return Promise.all(this.selectedDeliveryPoints.map(item => {
        return this.deleteDeliveryPoint(item)
        .then(res => {
          return true;
        });
      })).then(_ => {
        this.deliveryPoints = Object.assign([], this.unselectedDeliveryPoints);
    ...
    async goToPaging() {
      ...
      switch (this.activeMenu) {
        case 'all':
          res = await this.getAllDeliveryPoints(payload);
          break;
        case 'public':
          res = await this.getPublicDeliveryPoints(payload);
          break;
        case 'primary':
      ...
      this.deliveryPoints = Object.assign([], res.data);
      this.pagination = Object.assign({}, res.pagination);
    },
    selectAll() {
      let tmp = Object.assign([], this.deliveryPoints);
      if (tmp.length > 0) {
        tmp.forEach(element => {
          element.selected = true;
        });
        this.deliveryPoints = Object.assign([], tmp);
      }
    },
    unselectAll() {
      let tmp = Object.assign([], this.deliveryPoints);
    ...
    togglePublic(item) {
      if (item.public) {
        return this.unpublishDeliveryPoint(item).then(res => {
          item.public = false;
        });
      } else {
        return this.publishDeliveryPoint(item).then(res => {
          item.public = true;
        });
      }
    },
    togglePublicVisibility(item) {
      let tmp = Object.assign({}, item);
      if (item.publicVisibility == 'primary') {
        tmp.publicVisibility = 1;
      } else {
        tmp.publicVisibility = 0;
      }
      return this.togglePublicVisibilityOfDeliveryPoint(tmp).then(res => {
        item.publicVisibility = tmp.publicVisibility > 0 ? 'secondary' : 'primary';
      });
    },
    remove() {
      let index = this.deliveryPoints.indexOf(this.confirmBox.item);
      this.deleteLoading = true;
      return this.deleteDeliveryPoint(this.confirmBox.item).then(res => {
    ...
    createNewDelivery() {
      ...
      return this.addDeliveryPoint(this.form).then(res => {
        this.deliveryPoints.push(this.form);
    ...
    updateDelivery() {
      ...
      return this.updateDeliveryPoint(this.form).then(res => {
        this.deliveryPoints[index] = this.form;
        this.resetDeliveryForm();
        this.saveLoading = false;
      });
    }
  ...
  computed: {
    selectedDeliveryPoints() {
      return filter(this.deliveryPoints, element => element.selected);
    },
    unselectedDeliveryPoints() {
      return filter(this.deliveryPoints, element => !element.selected);
    },
    showBulkActionOptions() {
      return this.selectedDeliveryPoints && this.selectedDeliveryPoints.length > 0;
    }
  },
  watch: {
